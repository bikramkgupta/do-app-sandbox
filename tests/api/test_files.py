...
class TestFileOperations:
    ...
    def test_list_files(self, api_url, auth_headers):
        ...
        response = httpx.get(f"{api_url}/api/files?path=/workspace", headers=auth_headers, timeout=10.0)
        assert response.status_code == 200

        data = response.json()
        # API returns {"files": [...]} for extensibility
    ...
    def test_list_files_shows_metadata(self, api_url, auth_headers):
        ...
        # Find our file
        test_file = next((f for f in files if f.get("name") == "metadata_test.txt"), None)
    ...
    def test_write_file(self, api_url, auth_headers):
        ...
        response = httpx.post(
            ...
            json={"path": "/workspace/write_test.txt", "content": "written content here"},
            headers=auth_headers,
            timeout=10.0,
        )
        assert response.status_code == 200
    ...
    def test_write_creates_dirs(self, api_url, auth_headers):
        ...
        response = httpx.post(
            f"{api_url}/api/files/content",
            json={"path": "/workspace/nested/deep/dir/file.txt", "content": "nested file content"},
            headers=auth_headers,
            timeout=10.0,
        )
        assert response.status_code == 200
