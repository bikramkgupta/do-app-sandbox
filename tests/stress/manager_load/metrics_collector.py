...
class MetricsCollector:
    ...
    def take_snapshot(self, manager) -> PoolSnapshot:
        ...
        with self._lock:
            ...
            avg_latency = (
                sum(self._acquire_latencies) / len(self._acquire_latencies) if self._acquire_latencies else 0.0
            )
    ...
    def generate_summary(self) -> TestSummary:
        ...
        with self._lock:
            ...
            max_python = max(
                (s.python_ready + s.python_creating + s.python_in_use for s in self.snapshots),
                default=0,
            )
