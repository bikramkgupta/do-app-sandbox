...
from .config import LoadPattern, UserGroupConfig
from .metrics_collector import MetricsCollector, TaskResult
from .workload_generator import ProgramSpec, WorkloadGenerator
...
class UserSimulator:
    ...
    async def run(self, end_time: float):
        ...
        logger.info(
            f"User {self.user_id} finished: {self.state.tasks_completed} tasks, {self.state.tasks_failed} failed"
        )
    ...
    async def _run_program(
        ...
        if hasattr(sandbox, "filesystem"):
            ...
            await asyncio.to_thread(sandbox.filesystem.write_file, upload_path, program_content)
