...
class MetricsCollector:
    ...
    def record_pool_snapshot(
        ...
        with self._lock:
            ...
            if total_all > self.max_total:
                ...
                logger.critical(f"CAPACITY VIOLATION: {total_all} sandboxes (limit: {self.max_total})")
    ...
    async def sample_pool_metrics(
        ...
        while stop_event is None or not stop_event.is_set():
            try:
                ...
                # Log current state
                total = sum(
                    getattr(m, "ready", 0) + getattr(m, "creating", 0) + getattr(m, "in_use", 0)
    ...
    def get_summary(
        self,
        watchdog_metrics: WatchdogMetrics | None = None,
    ) -> TestSummary:
        """
        ...
        with self._lock:
            ...
            if self._start_time and self._end_time:
                summary.duration_seconds = (self._end_time - self._start_time).total_seconds()

            # Session counts
            summary.total_sessions = len(self._sessions)
            summary.total_commands = sum(s.total_commands_executed for s in self._sessions)
            summary.total_command_failures = sum(s.command_failures_count for s in self._sessions)
            ...
            for image in set(s.image for s in self._sessions):
                ...
                if image_sessions:
                    summary.pool_hit_rate_by_image[image] = image_pool_hits / len(image_sessions)

            # Latency percentiles
            all_latencies = [s.acquire_latency_ms for s in self._sessions]
            pool_latencies = [s.acquire_latency_ms for s in self._sessions if s.from_pool]
            cold_latencies = [s.acquire_latency_ms for s in self._sessions if not s.from_pool]
