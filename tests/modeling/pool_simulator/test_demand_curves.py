...
Run with:
    uv run --extra dev pytest tests/modeling/pool_simulator/test_demand_curves.py -v

Or run specific test class:
    uv run --extra dev pytest tests/modeling/pool_simulator/test_demand_curves.py::TestPoolHitRate -v
"""
...
class TestPoolHitRate:
    ...
    async def test_steady_load_within_capacity_has_pool_hits(self, small_pool_config):
        ...
        assert (
            result.successful_acquires > result.total_requests * 0.5
