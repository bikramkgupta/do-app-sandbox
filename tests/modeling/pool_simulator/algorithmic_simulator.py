...
class SimulatedPool:
    ...
    def __init__(
        ...
        target_ready: int,
        max_sandboxes: int,
        global_limit_callback: callable | None = None,
    ...
    async def acquire(self, timeout: float | None = None) -> SimulatedSandbox:
...
class AlgorithmicMockManager:
    ...
    def _total_sandbox_count(self) -> int:
        ...
        total = sum(p.ready_count + p.creating_count + p.in_use_count for p in self._pools.values())
        # Track max observed
    ...
    async def warm_up(self, timeout: float = 120.0) -> None:
        ...
        while time.time() - start_time < timeout:
            all_ready = all(p.ready_count >= p.target_ready for p in self._pools.values())
            if all_ready:
        ...
        # Timeout reached
        status = ", ".join(f"{img}: {p.ready_count}/{p.target_ready}" for img, p in self._pools.items())
        raise TimeoutError(f"Warm-up timeout after {timeout}s. Status: {status}")

    async def acquire(self, image: str, timeout: float | None = None) -> SimulatedSandbox:
